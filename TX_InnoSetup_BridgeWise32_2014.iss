...
begin
  ...
  ACAD2014 := RegvalueExists(HKLM, 'SOFTWARE\Autodesk\AutoCAD\R19.1\ACAD-D001:804','AcadLocation');
  ACAD2015 := RegvalueExists(HKLM, 'SOFTWARE\Autodesk\AutoCAD\R19.2\ACAD-E001:804','AcadLocation'); //----不知道
  ACAD2010 := False
  ACAD2011 := False
  ACAD2012 := False
  ACAD2013 := False
...
begin
  if CurStep=ssPostInstall then
     ...
     if  (CheckBox5.Checked) then
        begin
        RegWriteStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.1\ACAD-D001:804\Applications\BridgeWise', 'LOADER',  ExpandConstant('{app}')+'\TX_START.dll');
        RegWriteStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.1\ACAD-D001:804\Applications\BridgeWise', 'DESCRIPTION', '使BridgeWise随CAD启动！');
        RegWriteDWordValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.1\ACAD-D001:804\Applications\BridgeWise', 'LOADCTRLS', 2);
        RegWriteDWordValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.1\ACAD-D001:804\Applications\BridgeWise', 'MANAGED', 1);
        end;
     if  (CheckBox6.Checked) then
        begin
        RegWriteStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.2\ACAD-E001:804\Applications\BridgeWise', 'LOADER',  ExpandConstant('{app}')+'\TX_START.dll');
        RegWriteStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.2\ACAD-E001:804\Applications\BridgeWise', 'DESCRIPTION', '使BridgeWise随CAD启动！');
        RegWriteDWordValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.2\ACAD-E001:804\Applications\BridgeWise', 'LOADCTRLS', 2);
        RegWriteDWordValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Autodesk\AutoCAD\R19.2\ACAD-E001:804\Applications\BridgeWise', 'MANAGED', 1);
        end;
end;
...
begin
  ...
  CheckBox5.Left := ScaleX(80);
  CheckBox5.Top := CheckBox4.Top + ScaleY(30);
  CheckBox5.Width := Page.SurfaceWidth;
  CheckBox5.Height := ScaleY(17);
  CheckBox5.Caption := 'AutoCAD2014';
  CheckBox5.Parent := Page.Surface;
  CheckBox5.Enabled:= ACAD2014 ;
  ...
  CheckBox6 :=TCheckBox.Create(Page);
  CheckBox6.Left := ScaleX(80);
  CheckBox6.Top := CheckBox5.Top + ScaleY(30);
  CheckBox6.Width := Page.SurfaceWidth;
  CheckBox6.Height := ScaleY(17);
  CheckBox6.Caption := 'AutoCAD2015';
  CheckBox6.Parent := Page.Surface;
  CheckBox6.Enabled:= ACAD2015 ;
